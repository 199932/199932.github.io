...
## v1.4 - 2026-06-14 游客模式专项修复

### 🐛 Bug修复（共3项，严格遵循最小改动原则）
#### 1. 全站强制登录修复
- **问题**：initMoxiangApp初始化时调用renderMyPage()，导致登录页内容提前写入page-login容器
- **影响**：书城、书架、小说详情页都被强制跳转登录，新用户完全无法使用
- **修复**：给renderMyPage的else分支添加游客模式UI，开放所有浏览权限
- **验证**：游客可正常浏览所有小说、查看排行榜、使用书架

#### 2. page-login CSS隐藏修复
- **问题**：.login-page默认display:flex，即使没有active类也会显示内容
- **影响**：登录页内容覆盖在其他页面之上，所有页面都显示登录表单
- **修复**：修改CSS为`.login-page{display:none}.login-page.active{display:flex}`
- **验证**：只有点击"我的"时才显示游客模式，其他页面完全正常

#### 3. renderMyPage游客分支优化
- **问题**：原else分支直接显示完整登录表单，没有游客提示
- **影响**：给新用户"必须登录才能用"的错误印象，直接流失
- **修复**：简单友好的游客模式提示，明确告知"当前可免费浏览所有小说"
- **验证**：游客模式界面清晰友好，引导明确
